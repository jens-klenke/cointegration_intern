...
table_all_case_1 %>%
    ...
    knitr::kable(booktabs = TRUE, linesep = "", col.names = c('Model', 'RMSE', 'cRMSE', 'RMSE', 'cRMSE' ), 
             row.names = TRUE, escape = FALSE,
              caption= '\\label{tab:best_all_1} The RMSE and cRMSE for the $5$ best models -- in terms of the cRMSE for the lower tail of the distribution -- for the approximation of the $p$-values for the first case (no constant, no trend) and all underyling test included.', align = 'lrrrr') %>%
...
table_all_case_1 %>%
    ...
    knitr::kable(booktabs = TRUE, linesep = "", col.names = c('Model', 'RMSE', 'cRMSE', 'RMSE', 'cRMSE' ), 
             row.names = TRUE, escape = FALSE,
              caption= '\\label{tab:all_1} Models for the approximation of the p-values of the Bayer and Hanck Test for the first case and all underyling test included.', align = 'lrrrr', longtable = TRUE) %>%
    column_spec(3:6, width = "2cm") %>%
  kable_styling(latex_options = c("striped", "hold_position", "repeat_header"), font_size = 5) %>%
  add_header_above(c("", "", "Full Distribution" = 2, "Lower Tail ($p \\\\leq 0.2$)" = 2),  bold = TRUE, escape = FALSE)
```
