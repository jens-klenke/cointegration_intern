...
merges the tests using the formula \begin{align}
\tilde{\chi}^2_{\mathcal{I}} := -2 \sum_{i \in \mathcal{I}} \ln{(p_i)}.
\end{align} Let \(t_i\) be the \(i^{th}\) test statistic. If test \(i\)
rejects for large values, take \(\xi_i := t_i\). If test \(i\) rejects
for small values, take \(-\xi_i := t_i\). With
