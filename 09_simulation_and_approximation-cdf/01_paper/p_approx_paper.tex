...
RMSE, cRMSE, where predictions are corrected to be within the limited
interval {[}0, 1{]}.
...
Figure \ref{fig:fig_3} shows the prediction of the final models for all
...
sharply increase, tending towards 1. It should be noted that this upper
boundary is enforced by our build in correction for predicted values
outside the interval {[}0, 1{]}. Without this intervention the predicted
values would probably rise even further. If we predict on an extended
sequence with no correction, the prediction line most likely oscillates
