...
The package **bayerhanck** was created using `create_package()` from **devtools**. This generates an R project, files for the package metadata, such as `DESCRIPTION`, `NAMESPACE` and `LICENSE`, and a sub directory for the R code of subsequent functions. The individual functions were documented with the help of **roxygen2**. The roxygen comments in the source file were then converted to `.Rd` files in the sub directory `man/`. 
...
Here, the execution of `plot` accesses the test statistic of the `bayerhanck` function, the number of regressors and the trend type. It can therefore deduce, which null distribution is to be plotted. This information is then passed to the function `ggplot` from the package **ggplot2**. `ggplot::stat_ecdf()` is then used to plot the null distribution. In addition, the value of the Bayer-Hanck statistic is marked with a dotted x-intercept line.

# Illustration

The package shall be illustrated on the dataset `weather` from the package **nycflights13**. It contains the hourly meterological data for the airports LGA, JFK and EWR of the New York metropolitan area. We will test for a cointegration relationship between the variables *humid*, *temp* and *pressure*. 
