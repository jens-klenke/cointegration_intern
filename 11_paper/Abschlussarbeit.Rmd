...
The obtained values from the previously presented function `bayerhanck()` are based on critical values provided by the technical appendix of @Bayerhanck2009. As an extension to this function, the function `bayerhanck_1()` has been developed. It contains additional null distributions obtained from a specifically conducted simulation. Analogous to the basic version, it takes the form:
...
The only difference is that this functioncall of the bayerhanck test does not execute the test using the critical values we have gathered from  paper, but rather calculates the statistics with its own simulated values. Now that we have the simulated distributions under the $\mathcal{H}_0$ of the Bayer hanck statistics available we are able to calculate and print p-values. 

In the current implementation of the function, we have stayed with the initial options `eg-j` and `all` for the argument `test`. Mainly because the amount of data is already quite large and increases with each additional alternative test combination by about 7 MB. The main reason for this is that for each combination of regressors and trendtype we need to have a simulated $\mathcal{H}_0$ distribution with, in our case, 25 000 values. In order to reduce the amount of data and allow for further test combinations, the simulated distribution could be reduced to 10000 values per kombination of regressors and trendtype when the package is further developed.  

At the moment, the package already has a size of about 16 MB with only two test options. On the one hand the function needs the two null distributions for the bayerhanck statistics and on the other hand the null distribution of the underlying tests, which also has a size of 5 MB. 
