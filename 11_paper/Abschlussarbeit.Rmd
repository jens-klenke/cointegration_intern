...
needs to estimated first. Bayer and Hanck employ the test statistic $\lambda_{\max} (h) = - T \ ln(1 - \hat{\pi}_1)$, where $\hat{\pi}_1$ denotes the largest solution of $|\pi \pmb{S}_{11} - \pmb{S}_{10} \pmb{S}_{00}^{-1} \pmb{S}_{01}|= 0$.^[$\pmb{S}_{ij}$ denotes the moment matrices of reduced rank regression residuals.]
...
The package shall be illustrated on the dataset `weather` from the package **nycflights13** [@wickham_nycflights13_2020]. It contains the hourly meterological data for the airports LGA, JFK and EWR of the New York metropolitan area. We will test for a cointegration relationship between the variables *humid*, *temp* and *pressure*. These stand for the relative humidity, temperature and sea level pressure in millibars. 

The functions for the underlying tests will be demonstrated with a Engle-Granger test and a Johansen test:

```{r eval=FALSE}
eg_test <- englegranger(humid ~ temp + pressure,
                        data = nycflights13::weather)

jo_test <- johansen(humid ~ temp + pressure, 
                    data = nycflights13::weather)
```
...
Engle-Granger Test
----------------------------------------------------------
Formula: humid ~ temp + pressure
Lags: 1
Trend: const
 
Value of test statistic: -30.3546
```
...
Value of test statistic: 1186.6794
```
...
```{r eval = FALSE}
# test = "eg-j"
bh_eg_j <- bayerhanck(humid ~ temp + pressure, 
                      data = nycflights13::weather, 
                      test = "eg-j")

# test = "all"
bh_all <- bayerhanck((humid ~ temp + pressure, 
                      data = nycflights13::weather))
```
...
Test Statistics      -30.3546 1186.679
p-Values               0.0000    0.000
...
Test Statistics      -30.3546 1186.679 -18.0761 729.9936
p-Values               0.0000    0.000   0.0000   0.0000
 
Value of the Fisher Type Test statistic: 221.0482
...
```{r eval = FALSE}
# test = "eg-j"
bh_eg_j_1 <- bayerhanck_1(humid ~ temp + pressure, 
                          data = nycflights13::weather, 
                          test = "eg-j")

# test = "all"
bh_all_1 <- bayerhanck_1(humid ~ temp + pressure, 
                         data = nycflights13::weather)
...
Value of the Fisher Type Test statistic: 221.0482
5% Critical value for the Test statistic: 21.034 
P-value of the Fisher Type Test statistic: 0
```

To demonstrate the implemented method for the generic function `plot()`, we will additionally plot the object from `bayerhanck_1` for `test = "all"`. For a harmonoius appearance in this documentation, we will set `theme = "light"`.
