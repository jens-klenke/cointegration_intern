...
c
c Copyright (c) James G. MacKinnon, 1998; revised 2004-9-14
c DOS/Windows version of lrcdist.
c
c lrcdist.for:  This program estimates asymptotic critical values
c and P values for likelihood ratio cointegration tests.  It is written
c in Fortran 77.
c
c Those who wish to incorporate these routines into their own programs
c should use the routines in johrouts.f and lrcrouts.f instead.
c
...
c
c The routines must have access to the files probs.tab,
c joh-#.tab for # = 1, 2, 3 ..., 12, and lrc-#1-#2.tab
c for #1 = 01, 02, ..., 12 and #2=0, 1, ..., 8. As currently written,
c these files must be in the current directory or in the directory
c \URCDIST.
c
      character*1 yesno
c
      write(6,*) 'lrcdist. Copyright James G. MacKinnon, 1998, 2004.'
      write(6,*) ' '
c
      open (unit=13,file='lrcdist.out',status='OLD',err=590)
      write(6,*) 'File lrcdist.out exists. Overwrite it? y/n. '
      read(5,589) yesno
  589 format(1a1)
      ...
      stop
  590 open (unit=13,file='lrcdist.out')
c
  750 continue
      nxg = 0
c
      write(6,*) 'Are you using a Johansen test (no exogenous',
     &  'variables) or a PSS test (possibly exogenous variables)? '
      write(6,*) 'Enter 1 for Johansen, 2 for PSS: '
  30  read(5,*) itype
      if (itype.lt.1.or.itype.gt.2) then
        write(6,*) 'Please choose 1 or 2'
        go to 31
      end if
c
      write(6,*) 'How many restrictions are there? '
      write(6,*) 'This is p - r. It must be between 1 and 12. '
   31 read(5,*) npr
      if (npr.lt.1.or.npr.gt.12) then
        write(6,*) 'Number of restrictions must be between 1 and 12.'
        go to 31
      end if
      if (itype.eq.2) then
        write(6,*) 'How many exogenous variables are there? '
        write(6,*) 'It must be between 0 and 8. '
  35    read(5,*) nxg
        if (nxg.lt.0.or.nxg.gt.8) then
          write(6,*) 'Number of exogenous variables must be between',
     &      ' 0 and 8.'
          go to 35
        end if
      end if
c
      ...
      write(6,*) '1 ==> lambda-max test; 2 ==> trace test '
   32 read(5,*) itt
...
c
      if (itype.eq.1) then
        write(6,*) 'What case is this? '
        write(6,*) '0 ==> Case 0, 1 ==> Case 1, 2 ==> Case 2 '
        write(6,*) '3 ==> Case 1*, 4 ==> Case 2* '
  33    read(5,*) itv
        if (itv.lt.0.or.itv.gt.4) then
          write(6,*) 'The valid options are 0, 1, 2, 3, and 4.'
          go to 33
        end if
      end if
c
...
c
      if (itype.eq.2) then
        write(6,*) 'What case is this? '
        write(6,*) '1 ==> Case I, 2 ==> Case II, 3 ==> Case III '
        write(6,*) '4 ==> Case IV, 5 ==> Case V '
 36     read(5,*) itv
        if (itv.lt.1.or.itv.gt.5) then
          write(6,*) 'The valid options are 1, 2, 3, 4, and 5.'
          go to 36
        end if
        ihold = itv
        if (itv.eq.2) ihold = 3
...
c
c call johval or lrcval to do all the work
c
        if (itype.eq.1)then
          call johval(npr,itt,itv,nc,isave,size,cval)
        else
          call lrcval(npr,nxg,itt,itv,nc,isave,size,cval)
        end if
c
c write out results
c
        write(13,765) itype, npr, nxg, itt, itv, size, cval
  765   format(i1, i4, 3i2, f8.4, f10.4)
        write(6,89) size, cval
   89   format(' At level ', f6.4, ' asymptotic critical value = ',f9.4)
      else
...
c
         if (itype.eq.1)then
           call johval(npr,itt,itv,nc,isave,stat,pval)
         else
           call lrcval(npr,nxg,itt,itv,nc,isave,stat,pval)
         end if
c
c write out results
c
        write(13,766) itype, npr, nxg, itt, itv, stat, pval
  766   format(i1, i4, 3i2, f10.4, f8.4)
...
c directory or in \URCDIST. They could easily be changed to
...
c
  ...
  600 continue
c
c Change the next line if files are not in \URCDIST
c
      open (unit=12,file='\URCDIST\probs.tab',
     &  status='OLD',err=602)
  601 continue
...
c
  ...
  605 continue
     ...
     &  '\URCDIST.'
      ...
      subroutine lrcval(npr,nxg,itt,itv,nc,isave,arg,val)
c
c npr = number of restrictions (p - r): 1 to 12.
c nxg = number of exogenous variables: 0 to 8.
c itt = 1 or 2 for lambda-max test or trace test
c itv = 1, 2, 3, 4, or 5 for cases I (0), II (1*), III, IV (2*), or V.
c nc = 1 or 2 for critical value or P value
c isave = 0 or 1. If isave=1, this call to johval has the same values of
c                 npr, itt, and itv as the last one, so data files do not
c                 need to be read again. Be careful using this!
...
c
     ...
     &   ' and 12.'
      ...
      if (nxg.lt.0.or.nxg.gt.8) then
        write(6,*) 'Error! Number of exogenous variables must be',
     &    ' between 0 and 8.'
       stop
      end if
      if (itt.lt.1.or.itt.gt.2) then
      ...
      if (itv.lt.1.or.itv.gt.5) then
        write(6,*) 'Error! The valid options for itv are 1, 2, 3, 4,',
     &    ' and 5.'
       stop
      end if
      if (nc.lt.1.or.nc.gt.2) then
...
c
  ...
  601 continue
      if (npr.lt.10) then
        write(fname,200) npr, nxg
      else
        write(fname,201) npr,nxg
      end if
 200  format('lrc-0',i1,'-',i1,'.tab')
 201  format('lrc-',i2,'-',i1,'.tab')
      open (unit=11,file=fname,status='OLD',err=603)
      go to 604
  603 continue
c
c Change the next line if files are not in \URCDIST
...
c skip groups of 222 lines as necessary.
c
      if (itt.ne.1.and.npr.gt.1) iskip = 1110
...
c
      if (mform.eq.1) then
        do i=1,221
          ii = 222 - i
          read(11,303) crits(ii), wght(ii)
 303      format(f8.4, f9.6)
        end do
      else
        do i=1,221
          ii = 222 - i
          read(11,304) crits(ii), wght(ii)
 304      format(2d12.6)
        end do
      end if
      do i=1,221
...
c
  ...
  602 continue
     ...
     &  '\URCDIST.'
      stop
  605 continue
      write(6,*) 'Sorry. File ', fname, ' not found.'
      write(6,*) 'Please check current directory and ',
     &  '\URCDIST.'
      stop
      end
