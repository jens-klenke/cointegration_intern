c Copyright (c) James G. MacKinnon, 1998 (corrected 2001-5-05)
c 
c lrcrouts.f:  This is a set of subroutines to estimate asymptotic
c critical values and P values for likelihood ratio cointegration tests.
c It is written in Fortran 77.  Simply call lrcval, specifying the first
c seven arguments. The result comes back in the last argument.
c
c A standalone program called lrcdist.f, which incorporates these routines,
c is also available.
c
...
c
c The routines must have access to the files probs.tab and lrc-#1-#2.tab
...
c
      usrfile = '/usr/local/urcdist/' // fname
