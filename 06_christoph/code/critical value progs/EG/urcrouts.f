c Copyright (c) James G. MacKinnon, 1996 (corrected 2003-5-5)
c
...
c
      subroutine urcval(niv,itt,itv,nc,nobs,isave,arg,val)
c
...
c nobs = sample size (0 for asymptotic)
c isave = 0 or 1. If isave=1, this call to urcval has the same values of
c                 niv, itt, and itv as the last one, so data files do not
...
c ** Note that arg and val are double precision (real*8) variables **
c
      implicit real*8 (a-h,o-z)
      character*12 fnames(12)
      character*40 usrfile
      real*8 probs(221), cnorm(221), beta(4,221), wght(221)
      common/saveall/ probs, cnorm, beta, wght, nz, nreg, model, minsize
c
c common block saveall should not appear in any other routine
c
      fnames(1) = 'urc-1.tab'
      fnames(2) = 'urc-2.tab'
      fnames(3) = 'urc-3.tab'
...
c
      if (niv.lt.1.or.niv.gt.12) then
       write(6,*) 'Error! Number of variables must be between 1 and 12.'
       stop
      end if
      ...
      if (itv.lt.1.or.itv.gt.4) then
       write(6,*) 'Error! The valid options for itv are 1, 2, 3, and 4.'
       stop
      end if
