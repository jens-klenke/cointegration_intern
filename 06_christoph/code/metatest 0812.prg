...
/* DGP: */
K = 2;
T = 100; /* # time series observations */
...
for gam (1,rows(gamma_ma),1);
    size_mat = arrayinit(rows(T)|rows(rho)|noq|rows(a1)|1|1,4);
    size_mat2 = arrayinit(rows(T)|rows(rho)|noq|rows(a1)|1|1,4);
    size_mat_aeg = arrayinit(rows(T)|rows(rho)|noq|rows(a1)|1|1,4);
    size_mat_joh = arrayinit(rows(T)|rows(rho)|noq|rows(a1)|1|1,4);
    size_mat_aeg_mc = arrayinit(rows(T)|rows(rho)|noq|rows(a1)|1|1,4);
    size_mat_joh_mc = arrayinit(rows(T)|rows(rho)|noq|rows(a1)|1|1,4);
    rej_mat_bootstrap = arrayinit(rows(T)|rows(rho)|noq|rows(a1)|rep|1,4);
    rej_mat_bootstrap2 = arrayinit(rows(T)|rows(rho)|noq|rows(a1)|rep|1,4);
    rej_mat_bootstrap_aeg = arrayinit(rows(T)|rows(rho)|noq|rows(a1)|rep|1,4);
    rej_mat_bootstrap_joh = arrayinit(rows(T)|rows(rho)|noq|rows(a1)|rep|1,4);
    rej_mat_bootstrap_aeg_mc = arrayinit(rows(T)|rows(rho)|noq|rows(a1)|rep|1,4);
    rej_mat_bootstrap_joh_mc = arrayinit(rows(T)|rows(rho)|noq|rows(a1)|rep|1,4);
    for rh (1,rows(rho),1);
    for tt (1,rows(T),1);
    for endog (1,rows(a1),1);
        ...
        for qq (1,rows(q),1);
            for jj (1,rep,1);
                state = -1;
                {R,state} = rndKMn((T[tt]+discarded)*1,K,state);
                R2 = R*L;
                u1 = (reshape(R2[.,1],1,T[tt]+discarded))';
            
                u2 = zeros(1,1)|(reshape(R2[.,2],1,T[tt]+discarded))';
                u2 = u2[2:rows(u2),.] + gamma_ma[gam]*u2[1:(rows(u2)-1),.];
               
                z = recserar(u1,zeros(1,1),rho[rh]*ones(1,1)); /* with rho<1 you get a power study */
                w = recserar(u2,zeros(1,1),ones(1,1));  /* w has a unit root. */
              
                x = ((a2-a1[endog]*beta)^-1).*(a1[endog]*reshape(alfa,rows(z),1)+(a1[endog]*z-w)); /* figure out whether constant is appropriate */
                y = ((a2-a1[endog].*beta)^-1).*(a2*reshape(alfa,rows(z),1)+(a2*z-beta.*w));
                ...
                {phihat,sighat} = yule(diff(res,1),q_opt);
                /* residual series */
                phi_times_dyk = zeros(rows(diff(res,1))-q_opt,q_opt);
                all_eps_res[q_opt+1:rows(all_eps_res),1] = trimr(diff(res,1),q_opt,0);
                for j (1,q_opt,1);
                    phi_times_dyk[.,j] = phihat[j]*trimr(diff(res,1),q_opt-j,j);
                    all_eps_res[q_opt+1:rows(all_eps_res),1] = all_eps_res[q_opt+1:rows(all_eps_res),1] - phi_times_dyk[.,j];
                endfor;
                ...
                /* centered residuals*/
                all_eps_res_cen = all_eps_res_cut - reshape(meanc(all_eps_res_cut)',rows(all_eps_res_cut),1);
                ...
                for bb (1,B,1);
                    ...
                    what_star = recserar(all_eps_res_star,zeros(q[qq],1),phihat);
                ...
                p_joh_star = (B+1-rankindx(joh_star,1))/B;
                
                p_aeg = sumc(aeg_star .lt stat_aeg*ones(B,1))/B;
                p_Joh = sumc(joh_star .gt stat_joh*ones(B,1))/B;
                chisquarestat2 = -2*ln(p_aeg*p_Joh);

                fisherstat_star = -2*ln(p_aeg_star.*p_joh_star);

                bootstrap_pval = sumc(fisherstat_star .> chisquarestat*ones(B,1))/B;
                bootstrap_pval2 = sumc(fisherstat_star .> chisquarestat2*ones(B,1))/B;

                /* record rejections here */
                rej_mat_bootstrap[tt,rh,qq,endog,jj,1] = (bootstrap_pval lt 0.05);
                rej_mat_bootstrap2[tt,rh,qq,endog,jj,1] = (bootstrap_pval2 lt 0.05);

                rej_mat_bootstrap_aeg[tt,rh,qq,endog,jj,1] = (p_aeg lt 0.05);
                rej_mat_bootstrap_joh[tt,rh,qq,endog,jj,1] = (p_joh lt 0.05);

                rej_mat_bootstrap_aeg_mc[tt,rh,qq,endog,jj,1] = (pvalues_adf[1] lt 0.05);
                rej_mat_bootstrap_joh_mc[tt,rh,qq,endog,jj,1] = (pvalues_adf[2] lt 0.05);
                                                        
            endfor; /* end rep loop */
            format 8,2;
            print "-------------------------------------------------------------------";
            print "Case: rho=" rho[rh] " gamma=" gamma_ma[gam] " q=" q[qq] " T=" T[tt] " a1=" a1[endog]; 
            print "-------------------------------------------------------------------";
            format 8,4;
            size_mat[tt,rh,qq,endog,1,1] = meanc(rej_mat_bootstrap[tt,rh,qq,endog,.,1]);
            size_mat2[tt,rh,qq,endog,1,1] = meanc(rej_mat_bootstrap2[tt,rh,qq,endog,.,1]);
            size_mat_aeg[tt,rh,qq,endog,1,1] = meanc(rej_mat_bootstrap_aeg[tt,rh,qq,endog,.,1]);
            size_mat_joh[tt,rh,qq,endog,1,1] = meanc(rej_mat_bootstrap_joh[tt,rh,qq,endog,.,1]);
            size_mat_aeg_mc[tt,rh,qq,endog,1,1] = meanc(rej_mat_bootstrap_aeg_mc[tt,rh,qq,endog,.,1]);
            size_mat_joh_mc[tt,rh,qq,endog,1,1] = meanc(rej_mat_bootstrap_joh_mc[tt,rh,qq,endog,.,1]);
            print "P_chi^2 bootstrap rejection rate: " size_mat[tt,rh,qq,endog,1,1];
            print "P_chi^2 bootstrap rejection rate alternativ: " size_mat2[tt,rh,qq,endog,1,1];
            print "aeg power" size_mat_aeg[tt,rh,qq,endog,1,1];
            print "Jo power" size_mat_joh[tt,rh,qq,endog,1,1];
            print "aeg power mc" size_mat_aeg_mc[tt,rh,qq,endog,1,1];
            print "Jo power mc" size_mat_joh_mc[tt,rh,qq,endog,1,1];
            print "";
            ...
            filename = "st" $+ day_st $+ "_" $+ mth_st $+ "_" $+ hr_st $+ "h" $+ min_st $+ "cpT" $+ curr_T $+ "rho" $+ curr_rho $+ "q" $+ curr_q $+ "gam" $+ curr_gam $+ "a1_" $+ curr_a1 $+ "d" $+ day $+ "_" $+ mth $+ "_" $+ hr $+ "h" $+ min $+ ".asc";
            ...
            format 8,4;
            print "------------------------------------------------------------";
            print "bootstrap rejection rates:";
            print "------------------------------------------------------------";
            print "read: T,rho,fitted lag length,endogeneity";
            print size_mat;
            output off;
            screen on;
